...
import ThreadedPermutations

def test_empty_input():
    with pytest.raises(Exception) as e_info:
        ThreadedPermutations.generatePermutationsThreaded([])

def test_too_large_input():
    with pytest.raises(Exception) as e_info:
        ThreadedPermutations.generatePermutationsThreaded(["A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K", "L"])

def test_input_size_three():
    result = ThreadedPermutations.generatePermutationsThreaded(["A", "B", "C"])
    assert result == ['ABC', 'ACB', 'BAC', 'BCA', 'CAB', 'CBA']
    assert len(result) == 6

def test_input_size_five():
    result = ThreadedPermutations.generatePermutationsThreaded(["A", "B", "C", "D", "E"])
...
def test_input_size_eleven():
    result = ThreadedPermutations.generatePermutationsThreaded(["A", "B", "C", "D", "E", "F", "G", "H", "I", "J", "K"])
